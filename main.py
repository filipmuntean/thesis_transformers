...
class Main():
    ...
    # Sort the reviews
    sorted_reviews = util.loading.sort_reviews(i2w, x_train, y_train) #y_train
    ...
    padded_reviews = util.loading.get_padded_sequence(batched_sorted_reviews) #y_train
    
    # Build tensors
    padded_tensors = util.loading.get_tensor(padded_reviews)

    sentiment_tensors = util.loading.get_sentiment_tensor(y_train)
